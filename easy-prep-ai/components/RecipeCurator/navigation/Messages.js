...
const Messages = () => {
  ...
  const currentUserID = 'YOUR_CURRENT_USER_ID'; // This should be dynamically fetched based on logged-in user

  useEffect(() => {
    if (receiverId && currentUserID) {
      const messagesRef = collection(db, 'Messages');
      // Update the query to be more explicit in checking for messages between two users
      const q = query(
        messagesRef,
        orderBy('Time', 'asc'),
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={[
            ...
            item.SenderUser.id === currentUserID ? styles.myMessage : styles.theirMessage
          ]}>
